/// English (US) language translations for shirah
/// All text constants for the application in English
const Map<String, String> enUS = {
  // ==================== App Info ====================
  'app_title': 'SHIRAH',
  'app_tagline': 'Earn, Save & Connect',

  // ==================== Common ====================
  'error': 'Error',
  'cancel': 'Cancel',
  'ok': 'OK',
  'try_again': 'Try Again',
  'loading': 'Loading...',
  'save': 'Save',
  'done': 'Done',
  'back': 'Back',
  'next': 'Next',
  'skip': 'Skip',
  'continue': 'Continue',
  'submit': 'Submit',
  'confirm': 'Confirm',
  'close': 'Close',
  'search': 'Search',
  'filter': 'Filter',
  'sort': 'Sort',
  'refresh': 'Refresh',
  'see_all': 'See All',
  'view_all': 'View All',
  'view_more': 'View More',
  'no_data': 'No data available',
  'no_results': 'No results found',
  'coming_soon': 'Coming Soon',
  'offline': 'You are offline',

  // ==================== Home Screen ====================
  'home_title': 'Home',
  'welcome_message': 'Welcome to SHIRAH!',
  'welcome_subtitle': 'Earn rewards, save money, and connect with community',
  'good_morning': 'Good Morning',
  'good_afternoon': 'Good Afternoon',
  'good_evening': 'Good Evening',
  'total_balance': 'Total Balance',
  'quick_actions': 'Quick Actions',
  'recent_activity': 'Recent Activity',

  // ==================== Onboarding ====================
  'choose_theme': 'Choose Your Theme',
  'choose_theme_subtitle':
      'We recommend dark theme - easier on eyes and saves battery. You can change it later in settings.',
  'dark_theme': 'Dark',
  'light_theme': 'Light',
  'system_theme': 'System',
  'dark_theme_desc': 'Perfect for night time',
  'light_theme_desc': 'Bright and clear',
  'system_theme_desc': 'Follow system settings',
  'choose_language': 'Choose preferred language',
  'choose_language_subtitle':
      'SHIRAH comes with both English and Bangla languages. Choose your preferred language to continue.',
  'english': 'English',
  'bangla': 'বাংলা',
  'english_desc': 'English (US)',
  'bangla_desc': 'বাংলা (বাংলাদেশ)',
  'get_started': 'Get Started',

  // Style Selection
  'choose_style': 'Choose Your Style',
  'choose_style_subtitle':
      'Personalize your app with a unique color theme. You can change it anytime in settings.',
  'style_shirah': 'SHIRAH',
  'style_quepal': 'Quepal',
  'style_timber': 'Timber',
  'style_flare': 'Flare',
  'style_amin': 'Amin',
  'style_midnight': 'Midnight',
  'style_changed': 'App style changed',

  // ==================== Authentication ====================
  'login': 'Login',
  'login_title': 'Welcome Back',
  'login_subtitle': 'Sign in to continue',
  'phone_number': 'Phone Number',
  'enter_phone': 'Enter your phone number',
  'phone_hint': '01XXXXXXXXX',
  'invalid_phone': 'Invalid phone number',
  'send_otp': 'Send OTP',
  'verify_otp': 'Verify OTP',
  'enter_otp': 'Enter OTP',
  'otp_sent': 'OTP sent to your phone',
  'otp_subtitle': 'We\'ve sent a 6-digit code to',
  'resend_otp': 'Resend OTP',
  'resend_in': 'Resend in',
  'invalid_otp': 'Invalid OTP',
  'verify': 'Verify',
  'logout': 'Logout',
  'logout_confirm': 'Are you sure you want to logout?',

  // Invite Code
  'have_invite_code': 'Have an invite code?',
  'enter_invite_code': 'Enter Invite Code',
  'invite_code_optional': 'Invite code (optional)',
  'invalid_invite_code': 'Invalid invite code',

  // Profile Setup
  'setup_profile': 'Setup Profile',
  'setup_profile_subtitle': 'Tell us about yourself',
  'your_name': 'Your Name',
  'enter_name': 'Enter your name',
  'upload_photo': 'Upload Photo',
  'change_photo': 'Change Photo',

  // New Auth Flow
  'auth_sign_in_title': 'SIGN IN TO YOUR ACCOUNT',
  'auth_sign_in_subtitle': 'Enter your email and password to log in',
  'auth_email_hint': 'Email',
  'auth_password_hint': 'Password',
  'auth_password': 'Password',
  'auth_remember_me': 'Remember Me',
  'auth_forgot_password': 'Forgot Password?',
  'auth_log_in': 'LOG IN',
  'auth_no_account': 'Don\'t have an account? ',
  'auth_sign_up': 'SIGN UP',
  'auth_or_login_with': 'Or login with',
  'auth_or': 'Or',
  'auth_first_name': 'First Name',
  'auth_last_name_optional': 'Last Name (Optional)',
  'auth_phone_hint': 'Phone Number',
  'auth_confirm_password_hint': 'Confirm Password',
  'auth_have_account': 'Already have an account? ',
  'auth_forgot_password_title': 'FORGOT PASSWORD?',
  'auth_forgot_password_desc':
      'Don\'t worry! It happens. Please enter the email address associated with your account.',
  'auth_email_address': 'Email Address',
  'auth_send_reset_link': 'SEND RESET LINK',
  'auth_back_to_login': 'Back to Login',
  'auth_check_email_title': 'CHECK YOUR EMAIL!',
  'auth_check_email_subtitle': 'We\'ve sent a password reset link to:',
  'auth_check_email_instructions':
      'Please check your inbox and click the reset link to create a new password. If you don\'t see the email, check your spam folder.',
  'auth_tip_label': 'Tip:',
  'auth_check_email_tip':
      'The reset link will expire in 1 hour. If expired, request a new one.',
  'auth_didnt_receive_email': 'Didn\'t receive the email? ',
  'auth_resend': 'Resend',
  'auth_complete_profile': 'COMPLETE YOUR PROFILE',
  'auth_complete_profile_desc':
      'Enter your phone number and invite code to complete your profile.',
  'auth_complete_signup': 'COMPLETE SIGNUP',
  'auth_invite_code_tip':
      'Ask the person who invited you for their invite code. Every account needs one!',
  'auth_cancel_signup': 'Cancel Signup',
  'auth_cancel_signup_desc':
      'Are you sure you want to cancel? Your Google account will be disconnected and you\'ll need to start over.',
  'auth_stay': 'Stay',
  'auth_leave': 'Leave',
  'auth_login_success': 'Logged in successfully',
  'auth_signup_success': 'Account created successfully',
  'auth_password_reset_sent': 'Password reset email sent',
  'auth_logout_success': 'Logged out successfully',
  'auth_validating_invite_code': 'Validating invite code...',
  'auth_creating_account': 'Creating your account...',

  // ==================== Wallet ====================
  'wallet': 'Wallet',
  'wallet_balance': 'Wallet Balance',
  'reward_points': 'Reward Points',
  'available_balance': 'Available Balance',
  'total_earnings': 'Total Earnings',
  'withdraw': 'Withdraw',
  'deposit': 'Deposit',
  'transfer': 'Transfer',
  'convert_points': 'Convert Points',

  // Transactions
  'transactions': 'Transactions',
  'transaction_history': 'Transaction History',
  'no_transactions': 'No transactions yet',
  'pending': 'Pending',
  'completed': 'Completed',
  'failed': 'Failed',
  'cancelled': 'Cancelled',

  // Withdrawal
  'withdraw_funds': 'Withdraw Funds',
  'withdraw_to': 'Withdraw To',
  'enter_amount': 'Enter Amount',
  'min_withdraw': 'Minimum withdrawal: ৳50',
  'withdraw_method': 'Withdrawal Method',
  'account_number': 'Account Number',
  'confirm_withdraw': 'Confirm Withdrawal',
  'withdrawal_success': 'Withdrawal request submitted',
  'insufficient_balance': 'Insufficient balance',

  // Points Conversion
  'convert_to_bdt': 'Convert to BDT',
  'points_to_convert': 'Points to Convert',
  'you_will_receive': 'You will receive',
  'conversion_rate': '100 points = ৳1',
  'min_conversion': 'Minimum: 100 points',
  'convert': 'Convert',
  'conversion_success': 'Points converted successfully',
  'insufficient_points': 'Insufficient points',

  // ==================== Rewards ====================
  'rewards': 'Rewards',
  'earn_rewards': 'Earn Rewards',
  'your_rewards': 'Your Rewards',
  'watch_ads': 'Watch Ads',
  'watch_earn': 'Watch & Earn',
  'daily_rewards': 'Daily Rewards',

  // Streak
  'streak': 'Streak',
  'current_streak': 'Current Streak',
  'longest_streak': 'Longest Streak',
  'days': 'Days',
  'day_streak': 'Day Streak',
  'keep_streak': 'Keep your streak alive!',
  'streak_bonus': 'Streak Bonus',
  'multiplier': 'Multiplier',

  // Ads
  'ads_watched_today': 'Ads Watched Today',
  'points_earned_today': 'Points Earned Today',
  'daily_limit': 'Daily Limit',
  'ads_remaining': 'Ads Remaining',
  'watch_ad_earn': 'Watch ad to earn',
  'ad_not_ready': 'Ad not ready, try again later',
  'daily_limit_reached': 'Daily limit reached',

  // ==================== Recharge ====================
  'recharge': 'Recharge',
  'mobile_recharge': 'Mobile Recharge',
  'recharge_now': 'Recharge Now',
  'enter_number': 'Enter mobile number',
  'select_operator': 'Select Operator',
  'select_amount': 'Select Amount',
  'custom_amount': 'Amount',
  'recharge_success': 'Recharge successful',
  'recharge_failed': 'Recharge failed',
  'processing_recharge': 'Processing recharge...',
  'confirm_recharge': 'Confirm Recharge',
  'recent_history': 'Recent History',
  'cashback': 'Cashback',
  'prepaid': 'Prepaid',
  'postpaid': 'Postpaid',

  // ==================== Offers ====================
  'offers': 'Offers',
  'telecom_offers': 'Telecom Offers',
  'hot_offers': 'Hot Offers',
  'internet_packs': 'Internet Packs',
  'minute_packs': 'Minute Packs',
  'combo_packs': 'Combo Packs',
  'bundle_offers': 'Bundle Offers',
  'validity': 'Validity',
  'buy_now': 'Buy Now',
  'drive_offers': 'Drive Offers',
  'no_offers_found': 'No offers found',
  'sms_packs': 'SMS Packs',

  // ==================== Reselling ====================
  'reselling': 'Reselling',
  'products': 'Products',
  'my_orders': 'My Orders',
  'share_earn': 'Share & Earn',
  'product_commission': 'Commission',
  'order_placed': 'Order Placed',
  'order_shipped': 'Order Shipped',
  'order_delivered': 'Order Delivered',

  // ==================== Micro Jobs ====================
  'micro_jobs': 'Micro Jobs',
  'available_jobs': 'Available Jobs',
  'my_tasks': 'My Tasks',
  'job_details': 'Job Details',
  'apply_now': 'Apply Now',
  'task_submitted': 'Task Submitted',
  'earn_upto': 'Earn up to',

  // ==================== Community ====================
  'community': 'Community',
  'posts': 'Posts',
  'create_post': 'Create Post',
  'marketplace': 'Marketplace',
  'whats_on_mind': 'What\'s on your mind?',
  'post': 'Post',
  'like': 'Like',
  'comment': 'Comment',
  'share': 'Share',
  'comments': 'Comments',
  'write_comment': 'Write a comment...',

  // ==================== Profile ====================
  'profile': 'Profile',
  'edit_profile': 'Edit Profile',
  'my_profile': 'My Profile',
  'account': 'Account',
  'personal_info': 'Personal Information',
  'update_profile': 'Update Profile',
  'profile_updated': 'Profile updated successfully',

  // Invite & Referral
  'invite_friends': 'Invite Friends',
  'referrals': 'Referrals',
  'your_invite_code': 'Your Invite Code',
  'share_invite': 'Share Invite',
  'copy_code': 'Copy Code',
  'code_copied': 'Code copied to clipboard',
  'total_referrals': 'Total Referrals',
  'referral_earnings': 'Referral Earnings',

  // Settings
  'settings': 'Settings',
  'notifications': 'Notifications',
  'language': 'Language',
  'theme': 'Theme',
  'about': 'About',
  'help_support': 'Help & Support',
  'privacy_policy': 'Privacy Policy',
  'terms_conditions': 'Terms & Conditions',
  'rate_app': 'Rate App',
  'app_version': 'App Version',

  // ==================== Verification ====================
  'verify_account': 'Verify Account',
  'verified': 'Verified',
  'not_verified': 'Not Verified',
  'complete_verification': 'Complete verification to unlock all features',
  'verification_pending': 'Verification Pending',
  'verification_approved': 'Verification Approved',
  'verification_rejected': 'Verification Rejected',
  'verification_screen_title': 'Premium Account',
  'verification_already_done': 'Your account is already verified.',
  'verification_plan_title': 'Account Verification',
  'verification_plan_subtitle': 'One-time payment · Lifetime access',
  'verification_one_time': 'One-time · Lifetime',
  'verification_lifetime_note':
      'Both plans are one-time payments with lifetime access. No recurring charges.',
  'verification_get_verified': 'Get Verified Now',
  'verification_complete': 'Your account is fully verified',
  'verification_incomplete': 'Verify your account to unlock premium features',
  'verification_not_subscribed': 'Not Subscribed',
  'verification_benefits_title': 'What You Get',
  'verification_benefit_1': 'Earn Reward Points',
  'verification_benefit_1_desc': 'Get reward points from referral activities',
  'verification_benefit_2': 'Referral Rewards',
  'verification_benefit_2_desc':
      'Earn when your network grows and stays active',
  'verification_benefit_3': 'Exclusive Offers',
  'verification_benefit_3_desc': 'Access premium deals and cashback offers',
  'verification_benefit_4': 'Verified Badge',
  'verification_benefit_4_desc': 'Stand out with a verified profile badge',
  'verification_benefit_5': 'Priority Support',
  'verification_benefit_5_desc': 'Get faster customer support response',
  'verification_no_payments': 'No payment history yet',
  'verification_payment_history': 'Payment History',
  'verification_processing': 'Processing verification...',
  'verification_success': 'Account verified successfully!',
  'verify_first': 'Please verify your account first',
  'payment_config_error': 'Could not load payment configuration. Try again.',
  'payment_being_processed': 'Your payment is being processed.',
  'payment_pending_message':
      'Your payment is pending. We will notify you once confirmed.',
  'payment_cancelled_message': 'Payment was cancelled.',
  'payment_failed_message': 'Payment failed. Please try again.',

  // ==================== Subscription ====================
  'subscription': 'Subscription',
  'premium': 'Premium',
  'subscribe_now': 'Get Pro Access',
  'subscribed': 'Pro Active',
  'subscription_benefits': 'Subscription Benefits',
  'subscription_expires': 'Lifetime Access',
  'subscription_plan_title': 'Pro Subscription',
  'subscription_plan_subtitle': 'One-time payment · Lifetime access',
  'subscription_monthly': 'One-time · Lifetime',
  'subscription_already_active': 'Your subscription is already active.',
  'subscription_success': 'Pro subscription activated successfully!',

  // ==================== Error Messages ====================
  'network_error': 'Network error. Please check your connection.',
  'something_went_wrong': 'Something went wrong. Please try again.',
  'session_expired': 'Session expired. Please login again.',
  'permission_denied': 'Permission denied',
  'feature_locked': 'This feature is locked',

  // ==================== Success Messages ====================
  'success': 'Success',
  'saved_successfully': 'Saved successfully',
  'updated_successfully': 'Updated successfully',
  'deleted_successfully': 'Deleted successfully',

  // ==================== UI Actions ====================
  'toggle_theme': 'Toggle Theme',
  'toggle_language': 'Toggle Language',
  'theme_changed': 'Theme changed',
  'dark_theme_enabled': 'Dark theme enabled',
  'light_theme_enabled': 'Light theme enabled',
  'language_changed': 'Language changed',

  // ==================== Payment ====================
  'payment': 'Payment',
  'payment_method': 'Payment Method',
  'select_payment_method': 'Select Payment Method',
  'pay_now': 'Pay Now',
  'payment_successful': 'Payment Successful',
  'payment_failed': 'Payment Failed',
  'payment_cancelled': 'Payment Cancelled',
  'processing_payment': 'Processing Payment',

  // ==================== Onboarding Screens ====================
  'ob_welcome_title': 'Welcome to SHIRAH',
  'ob_welcome_subtitle': 'A Digital Platform for Skills and Services',
  'ob_welcome_desc':
      'SHIRAH brings learning, digital services, and community features together in one organized platform.',
  'ob_how_works_title': 'How It Works',
  'ob_how_works_subtitle': 'Simple Structure, Clear Flow',
  'ob_how_works_desc':
      'Explore different features, use available services, and participate in platform activities at your own pace.',
  'ob_transparency_title': 'Transparency & Control',
  'ob_transparency_subtitle': 'Designed for Clarity and Trust',
  'ob_transparency_desc':
      'All activities, settings, and account options are clearly presented so you always know how the platform works.',
  'ob_features_title': 'SHIRAH Features',
  'ob_features_subtitle': 'Multiple Features in One Platform',
  'ob_feature_1': 'Digital Learning Modules',
  'ob_feature_2': 'Product Reselling Hub',
  'ob_feature_3': 'Mobile Recharge & Top-Up',
  'ob_feature_4': 'Micro Jobs & Tasks',
  'ob_feature_5': 'Freelance Marketplace',
  'ob_feature_6': 'Exclusive Telecom Offers',
  'ob_feature_7': 'Health & Wellness Tips',
  'ob_feature_8': 'Reward Points System',
  'ob_feature_9': 'Community Networking',
  'ob_feature_10': 'Career Growth Insights',
  'ob_feature_11': 'Event & News Updates',
  'ob_feature_12': 'Achievement Badges',

  // ==================== Home Feed ====================
  'feed_empty': 'No feed items yet',
  'feed_empty_subtitle':
      'Your feed is empty. Posts and jobs will appear here once they are published.',
  'feed_error_subtitle':
      'Failed to load feed. Please check your connection and try again.',
  'feed_ad_badge': 'Ad',
  'feed_ad_loading': 'Loading ad...',
  'feed_sponsored_badge': 'Sponsored',
  'feed_announcement_badge': 'Announcement',
  'feed_micro_job_badge': 'Micro Job',
  'feed_community_post': 'Community Post',
  'feed_reselling': 'Product',
  'feed_drive_offer': 'Drive Offer',
  'feed_on_demand': 'Looking For',
  'feed_buy_sell': 'Buy & Sell',
  'feed_ads_view': 'Watch & Earn',
  'feed_ads_view_progress': 'ads watched today',
  'feed_ads_view_complete': 'Daily ads completed!',
  'feed_ads_view_remaining': 'ads remaining',
  'feed_suggested_following': 'People You May Know',
  'feed_load_more': 'Load More',
  'feed_end_of_feed': 'You\'re all caught up!',

  // ==================== Admin Feed Management ====================
  'admin_feed_title': 'Feed Management',
  'admin_feed_subtitle':
      'Manage home feed items, native ads, and content visibility',
  'admin_feed_all': 'All',
  'admin_feed_active': 'Active',
  'admin_feed_disabled': 'Disabled',
  'admin_feed_hidden': 'Hidden',
  'admin_feed_removed': 'Removed',
  'admin_feed_total_items': 'Total Items',
  'admin_feed_active_items': 'Active',
  'admin_feed_ads_count': 'Native Ads',
  'admin_feed_disabled_count': 'Disabled',
  'admin_feed_empty': 'No feed items found',
  'admin_feed_empty_subtitle':
      'Feed items will appear here once posts or jobs are approved.',
  'admin_feed_type': 'Type',
  'admin_feed_status': 'Status',
  'admin_feed_priority': 'Priority',
  'admin_feed_visibility': 'Visibility',
  'admin_feed_created_at': 'Created',
  'admin_feed_ref_id': 'Reference ID',
  'admin_feed_author_id': 'Author',
  'admin_feed_pinned': 'Pinned',
  'admin_feed_boosted': 'Boosted',
  'admin_feed_change_status': 'Change Status',
  'admin_feed_change_priority': 'Change Priority',
  'admin_feed_status_reason': 'Reason for status change',
  'admin_feed_status_updated': 'Feed item status updated',
  'admin_feed_priority_updated': 'Feed item priority updated',
  'admin_feed_confirm_remove':
      'Are you sure you want to remove this feed item?',
  'admin_feed_confirm_disable':
      'Are you sure you want to disable this feed item?',
  'admin_feed_detail_title': 'Feed Item Details',
  'admin_feed_filter_by_type': 'Filter by Type',
  'admin_feed_filter_by_status': 'Filter by Status',
  'admin_feed_search_hint': 'Search feed items...',
  'admin_feed_priority_low': 'Low',
  'admin_feed_priority_normal': 'Normal',
  'admin_feed_priority_important': 'Important',
  'admin_feed_priority_critical': 'Critical',
  'admin_feed_no_ref': 'No Reference',
  'admin_feed_actions': 'Actions',

  // ==================== Admin Native Ad Management ====================
  'admin_native_ad_title': 'Native Ad Management',
  'admin_native_ad_subtitle':
      'Create and manage native advertisement feed items',
  'admin_native_ad_create': 'Create Native Ad',
  'admin_native_ad_create_subtitle': 'Add a new native ad to the home feed',
  'admin_native_ad_unit_id': 'Ad Unit ID',
  'admin_native_ad_unit_id_hint': 'Enter AdMob ad unit ID',
  'admin_native_ad_platform': 'Platform',
  'admin_native_ad_platform_hint': 'Select target platform',
  'admin_native_ad_min_gap': 'Minimum Gap',
  'admin_native_ad_min_gap_hint': 'Min posts between ads (default: 6)',
  'admin_native_ad_max_per_session': 'Max Per Session',
  'admin_native_ad_max_per_session_hint': 'Max ads per session (default: 3)',
  'admin_native_ad_created': 'Native ad feed item created successfully',
  'admin_native_ad_android': 'Android',
  'admin_native_ad_ios': 'iOS',
  'admin_native_ad_both': 'Both',
  'admin_native_ad_rules': 'Ad Display Rules',
  'admin_native_ad_emergency_pause': 'Emergency Pause',
  'admin_native_ad_emergency_pause_desc': 'Temporarily pause all native ads',
  'admin_native_ad_active_ads': 'Active Ads',
  'admin_native_ad_no_ads': 'No native ads found',
  'admin_native_ad_no_ads_subtitle':
      'Create your first native ad to display in the feed.',

  // ==================== User Profile Screen ====================
  'profile_followers': 'Followers',
  'profile_following': 'Following',
  'profile_community_members': 'Community Members',
  'profile_sell_now': 'Sell Now',
  'profile_wholesale': 'Wholesale',
  'profile_max_selling': 'Max Selling',
  'profile_your_earning_upto': 'Your earning upto',
  'profile_per_sale': 'Per Sale',
  'profile_job_post': 'Job Post',
  'profile_colors': 'Colors:',
  'profile_sizes': 'Sizes:',
  'profile_tab_community': 'Community',
  'profile_tab_buy_sell': 'Buy & Sell',
  'profile_tab_job_post': 'Job Post',
  'profile_tab_products': 'On-Demand',
  'profile_bio_add': 'Add your bio...',
  'profile_bio_empty': 'Tell the world about yourself. Tap to add bio.',
  'profile_bio_edit': 'Edit Bio',
  'profile_bio_dialog_title': 'Update Your Bio',
  'profile_bio_hint': 'Share a little about yourself...',
  'profile_bio_save': 'Save Bio',
  'profile_bio_saving': 'Saving...',
  'profile_bio_saved': 'Bio updated successfully',
  'profile_bio_error': 'Failed to update bio',
  'profile_bio_max_chars': '150 characters max',

  // ── Image Picker ────────────────────────────────────────────
  'picker_profile_photo': 'Update Profile Photo',
  'picker_cover_photo': 'Update Cover Photo',
  'picker_gallery': 'Choose from Gallery',
  'picker_camera': 'Take a Photo',
  'picker_uploading_avatar': 'Uploading profile photo...',
  'picker_uploading_cover': 'Uploading cover photo...',
  'picker_avatar_success': 'Profile photo updated',
  'picker_cover_success': 'Cover photo updated',
  'picker_upload_failed': 'Upload failed. Please try again.',
  'picker_permission_denied': 'Permission Denied',
  'picker_permission_gallery_msg':
      'Please allow access to your photos in Settings to continue.',
  'picker_permission_camera_msg':
      'Please allow camera access in Settings to continue.',
  'picker_open_settings': 'Open Settings',

  // ── Edit Profile ─────────────────────────────────────────────
  'edit_profile_title': 'Edit Profile',
  'edit_first_name': 'First Name',
  'edit_last_name': 'Last Name',
  'edit_phone': 'Phone Number',
  'edit_phone_hint': 'Cannot be changed here',
  'edit_save_changes': 'Save Changes',
  'edit_profile_success': 'Profile updated successfully',
  'edit_profile_failed': 'Failed to update profile',
  'edit_name_required': 'Name is required',

  // ── Community ────────────────────────────────────────────────
  'community_members': 'Community Members',
};
